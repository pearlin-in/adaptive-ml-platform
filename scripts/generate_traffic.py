...
def random_fraud_payload(malformed: bool = False) -> dict:
    payload = {
...
def random_satellite_image(malformed: bool = False) -> bytes:
    if malformed:
        return b"not a real image"
    # Ensure explicit RGB uint8 array and explicit image encoding
...
def random_text_payload(malformed: bool = False) -> dict:
    return {"text": ""} if malformed else {"text": random.choice(SAMPLE_TEXTS)}


async def hit_fraud(client: httpx.AsyncClient, sem: asyncio.Semaphore):
    async with sem:
        try:
            r = await client.post(
...
async def hit_satellite(client: httpx.AsyncClient, sem: asyncio.Semaphore):
    async with sem:
...
async def hit_ai_text(client: httpx.AsyncClient, sem: asyncio.Semaphore):
    async with sem:
        try:
            r = await client.post(
                f"{BASE_URL}/predict/ai_text",
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for _ in range(REQUESTS_PER_MODEL):
            tasks += [
                hit_fraud(client, sem),
                hit_satellite(client, sem),
                hit_ai_text(client, sem),
            ]
